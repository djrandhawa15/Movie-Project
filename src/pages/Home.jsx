...
const Home = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState(
    localStorage.getItem("selectedCategory") || "popular"
  );
  ...
  const fetchMovies = async (categoryOrQuery, page = 1) => {
    try {
      ...
      const url = ["popular", "top_rated", "now_playing", "upcoming"].includes(
        categoryOrQuery
      )
      ...
      setMovies((prevMovies) =>
        page === 1 ? moviesWithPosters : [...prevMovies, ...moviesWithPosters]
      );
      ...
      if (moviesWithPosters.length > 0) {
        const randomIndex = Math.floor(
          Math.random() * moviesWithPosters.length
        );
